          ...
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-4 opacity-80">
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-4">
                <button onClick={() => append({ role: 'user', content: "Analyze my carbon footprint this month." })} className="bg-white/50 border border-white/50 px-4 py-2 rounded-full font-label-sm text-on-surface-variant hover:text-primary transition-colors">Analyze footprint</button>
                <button onClick={() => append({ role: 'user', content: "Generate 3 weekly sustainability goals." })} className="bg-white/50 border border-white/50 px-4 py-2 rounded-full font-label-sm text-on-surface-variant hover:text-primary transition-colors">Generate goals</button>
              </div>
            </div>
          )}
