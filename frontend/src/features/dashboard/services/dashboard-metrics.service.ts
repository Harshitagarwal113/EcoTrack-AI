...
export const getDashboardMetrics = cache(async function getDashboardMetrics() {
  ...
  try {
    ...
    const currentTotal = currentMonthEntries?.reduce((sum, e) => sum + Number(e.carbon_calculated), 0) || 0;
    ...
    const prevTotal = prevMonthEntries?.reduce((sum, e) => sum + Number(e.carbon_calculated), 0) || 0;
    ...
    if (goals && goals.length > 0) {
      const totalTarget = goals.reduce((sum, g) => sum + Number(g.target_reduction), 0);
      const currentTarget = goals.reduce((sum, g) => sum + Number(g.current_progress), 0);
      if (totalTarget > 0) {
