...
export async function getReportData(timeframe: 'weekly' | 'monthly') {
  ...
  try {
    ...
    const currentTotal = currentEntries?.reduce((sum, e) => sum + Number(e.carbon_calculated), 0) || 0;
    ...
    const prevTotal = prevEntries?.reduce((sum, e) => sum + Number(e.carbon_calculated), 0) || 0;
